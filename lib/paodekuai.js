...
function applyMove(state, player, move) {
  ...
  if (move.action === 'pass') {
    ...
    // 强制压牌: 有牌能压过时必须出牌
    const lastCards = state.last ? state.last.cards : null;
    if (lastCards) {
      const hint = Cards.findHint(state.hands[seat], lastCards);
      if (hint) {
        // 复验: 防止findHint误报导致玩家被双向卡死(出牌被拒+pass被拒)
        const hc = Cards.parse(hint);
        const lc = state.last.combo;
        if (hc && lc && Cards.beats(hc, lc)) {
          return { ok: false, errors: ['你有能压过的牌，必须出牌'] };
        }
      }
    }
    state.passCount += 1;
...
// 提示: 首手固定♠3, 其余findHint (强制压牌下, hint有解则必须出)
function hint(state, player) {
  if (!state || state.gameover) return { ok: false, errors: ['对局已结束'] };
  if (state.turn !== player) return { ok: false, errors: ['还没轮到你'] };
  const seat = player - 1;
  if (state.firstMove) {
    if (seat === state.firstSeat && state.hands[seat].indexOf(0) >= 0) return { ok: true, cards: [0] };
    return { ok: true, cards: [] };
  }
  const lastCards = state.last ? state.last.cards : null;
  const cards = Cards.findHint(state.hands[seat], state.freeTurn ? null : lastCards);
